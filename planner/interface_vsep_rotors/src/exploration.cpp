  ...
  if (triggerPlanner){
    ...
    if (ros::service::call("vseplanner", planSrv)) {
      ...
      for (int i = 0; i < planSrv.response.path.size(); i++) {
        ...
        trajectory_pubpnt->publish(samples_array);
...
int main(int argc, char** argv)
{
  ...
  ros::ServiceServer service = nh.advertiseService("/trigger_planner", trigger_planner);

  trajectory_pubpnt = &trajectory_pub;
  ...
  if (!ros::param::get(ns + "/automatic", automaticMode)) {
    if (automaticMode) ROS_INFO("AUTOMATIC EXPLORATION");
    else ROS_INFO("MANUAL EXPLORATION");
  }

  if (!ros::param::get(ns + "/initialization_enable", initEnable)) {
  }
