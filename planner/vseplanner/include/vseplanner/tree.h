...
public:
  ...
  double gain_;
  double distance_;
  int marker_; // mark a node unexplored or explored
...
public:
  ...
  void clearRootStateBBX(
      Eigen::Vector3d &boundingBox_,
      const double &minZ = -std::numeric_limits<double>::infinity());
  void getRootState(Eigen::Vector3d &rootState_);

  virtual void
  setStateFromPoseMsg(const geometry_msgs::PoseWithCovarianceStamped &pose) = 0;
  ...
  setPeerStateFromPoseMsg(const geometry_msgs::PoseWithCovarianceStamped &pose,
                          int n_peer) = 0;
  void setPeerStateFromPoseMsg1(
      const geometry_msgs::PoseWithCovarianceStamped &pose);
  void setPeerStateFromPoseMsg2(
      const geometry_msgs::PoseWithCovarianceStamped &pose);
  void setPeerStateFromPoseMsg3(
      const geometry_msgs::PoseWithCovarianceStamped &pose);
  ...
  virtual std::vector<geometry_msgs::Pose>
  getBestBranch(std::string targetFrame) = 0;
  virtual std::vector<geometry_msgs::Pose>
  ...
  // Publishing
  virtual void
  publishPath(vsExploration::PlanningLevel planninglevel = NBVP_PLANLEVEL) = 0;
  virtual void publishBestPath(
      vsExploration::PlanningLevel planninglevel = NBVP_PLANLEVEL) = 0;

  int getCounter();
  bool gainFound();
  bool reGainFound();
  void
  insertPointcloudWithTf(const sensor_msgs::PointCloud2::ConstPtr &pointcloud);
