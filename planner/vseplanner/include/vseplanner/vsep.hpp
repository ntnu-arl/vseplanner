...
#define ANGABS(x)                                                              \
  (fmod(fabs(x), 2.0 * M_PI) < M_PI ? fmod(fabs(x), 2.0 * M_PI)                \
                                    : 2.0 * M_PI - fmod(fabs(x), 2.0 * M_PI))
...
vsExploration::vsePlanner<stateVec>::vsePlanner(
  ...
  params_.planningWorkspace_ =
      nh_.advertise<visualization_msgs::Marker>("planningWorkspace", 1000);
  params_.planningPath_ =
      nh_.advertise<visualization_msgs::MarkerArray>("planningPath", 1000);
  params_.planningPathStats_ =
      nh_.advertise<visualization_msgs::MarkerArray>("planningPathStats", 1000);
  params_.bestPlanningPath_ =
      nh_.advertise<visualization_msgs::MarkerArray>("bestPlanningPath", 1000);
  params_.rePlanningPath_ =
      nh_.advertise<visualization_msgs::MarkerArray>("rePlanningPath", 1000);
  params_.rePlanningPathStats_ = nh_.advertise<visualization_msgs::MarkerArray>(
      "rePlanningPathStats", 1000);
  params_.bestRePlanningPath_ = nh_.advertise<visualization_msgs::MarkerArray>(
      "bestRePlanningPath", 1000);

  // Set up the topics and services
  params_.inspectionPath_ =
      nh_.advertise<visualization_msgs::Marker>("inspectionPath", 1000);
  plannerService_ = nh_.advertiseService(
      "vseplanner", &vsExploration::vsePlanner<stateVec>::plannerCallback,
      this);
  saveMapClient_ =
      nh_.serviceClient<volumetric_msgs::SaveMap>("vsePlanner/save_map");

  plannerPub_ = nh_.advertise<nav_msgs::Path>("planner_path", 1000);

  posClient_ = nh_.subscribe(
      "pose", 10, &vsExploration::vsePlanner<stateVec>::posCallback, this);
  odomClient_ = nh_.subscribe(
      "odometry", 10, &vsExploration::vsePlanner<stateVec>::odomCallback, this);

  pointcloud_sub_ = nh_.subscribe(
      "pointcloud_throttled", 1,
      &vsExploration::vsePlanner<stateVec>::insertPointcloudWithTf, this);
  ...
  pointcloud_sub_cam_down_ = nh_.subscribe(
      ...
      &vsExploration::vsePlanner<stateVec>::insertPointcloudWithTfCamDown,
      this);

  image_transport::ImageTransport it(nh_);
  subCam =
      it.subscribe("image_saliency", 1,
                   &vsExploration::vsePlanner<stateVec>::camCallback, this);
  subCamInfo = nh_.subscribe(
      "image_info", 2, &vsExploration::vsePlanner<stateVec>::camInfoCallback,
      this);
    ...
    if (stlPath.length() > 0) {
      if (ros::param::get(ns + "/mesh_resolution", params_.meshResolution_)) {
        ...
        if (stlFile.is_open()) {
          ...
          mesh_->setCameraParams(params_.camPitch_, params_.camHorizontal_,
                                 params_.camVertical_, params_.gainRange_);
        } else {
          ROS_WARN("Unable to open STL file");
        }
      } else {
        ROS_WARN("STL mesh file path specified but mesh resolution parameter "
                 "missing!");
      }
    }
  }
                                                          ...
                                                          vsep_msgs::vsep_srv::Response& res)
{
  ...
  if (!ros::ok()) {
    ROS_INFO_THROTTLE(1, "Exploration finished. Stop planning.");
    return true;
  }
  if (!ready_) {
  ...
  int planner_mode = 1; // 1: normal 2: saliency based planning
  static int target_counter = 0;
  if (params_.cuEnable_ && (tree_->getPlannerState() == PSTATE_EXPL_SAL)) {
    // Switch to saliency based exploration only when use curisosity mode
    // And the percentage of map explored is larger than a threshold
    planner_mode = 2;
  ...
  if ((!params_.cuEnable_) || (planner_mode != 2)) {
    ...
    while ((!tree_->gainFound() ||
            tree_->getCounter() < params_.initIterations_) &&
           ros::ok()) {
      ...
      if (loopCount > 1000 * (tree_->getCounter() + 1)) {
        ROS_WARN("planner<iterate>: Exceeding maximum failed iterations, return"
                 "to previous point!");
    ...
    // Saliency planner: entropy based
    // Build a tree with fixed number of vertices
    while (ros::ok() && (tree_->getCounter() < 50)) {
      if (loopCount > 1000 * (tree_->getCounter() + 1)) {
        ROS_WARN("planner<iterate>: Exceeding maximum failed iterations, return"
                 "to previous point!");
        res.path = tree_->getPathBackToPrevious(req.header.frame_id);
  ...
  // Resample first vertex of NBVP to improve the volumetric mapping gain.
  if (planner_mode != 2) {
  ...
  // Second-layer planner: saliency based
  computationTime = ros::Time::now();
  if (params_.cuEnable_) {
    if (planner_mode != 2) {
      bool res_resampleBestEdge = tree_->resampleBestEdge(ext_ratio);

      // Bsp: BSP planning level done, publish entire MarkerArray (full nested
      // replanning tree)
      ...
      bool res_ent = tree_->computeBestEntropyBranch();
      if (res_ent) {
        res.path = tree_->getBestBranch(req.header.frame_id);
        tree_->publishBestPath(BSP_PLANLEVEL);
        ROS_INFO("Finished 2nd layer: path computation lasted %2.3fs",
...
void vsExploration::vsePlanner<stateVec>::camCallback(
  ...
  if ((params_.sal_wait_for_planner_ &&
       (params_.planner_state_ >= PSTATE_INITIALIZED)) ||
...
bool vsExploration::vsePlanner<stateVec>::setParams() {
  ...
  if (!ros::param::get(ns + "/system/dyaw_sampling_limit",
                       params_.dyaw_sampling_limit_)) {
  ...
  if (!ros::param::get(ns + "/vsep/gain/probabilistic",
                       params_.igProbabilistic_)) {
    ROS_WARN("No gain coefficient for probability of cells specified. Looking "
             "for %s. Default is 0.0.",
  ...
  if (!ros::param::get(ns + "/vsep/gain/area", params_.igArea_)) {
    ROS_WARN("No gain coefficient for mesh area specified. Looking for %s. "
             "Default is 1.0.",
             (ns + "/vsep/gain/area").c_str());
  }
  params_.degressiveSwitchoffLoops_ = 5;
  if (!ros::param::get(ns + "/vsep/gain/degressive_switchoffLoops",
                       params_.degressiveSwitchoffLoops_)) {
    ROS_WARN("No Loop counts for degressive factor for gain accumulation "
             "specified. Looking for %s. Default is 5.",
             (ns + "/vsep/gain/degressive_switchoffLoops").c_str());
  }
  params_.degressiveCoeff_ = 0.25;
  if (!ros::param::get(ns + "/vsep/gain/degressive_coeff",
  ...
  if (!ros::param::get(ns + "/vsep/tree/extension_range",
  ...
  if (!ros::param::get(ns + "/vsep/tree/initial_iterations",
                       params_.initIterations_)) {
    ROS_WARN("No number of initial tree iterations specified. Looking for %s. "
             "Default is 15.",
  ...
  if (!ros::param::get(ns + "/vsep/yaw_sampling_limit",
                       params_.yaw_sampling_limit_)) {
    ROS_WARN(
        "No yaw sampling limit specified. Looking for %s. Default is 2*PI.",
  ...
  if (!ros::param::get(ns + "/bbx/plusX", params_.plusX_)) {
    ROS_WARN("No plusX value specified. Looking for %s",
             (ns + "/bbx/plusX").c_str());
    ret = false;
  }

  if (!ros::param::get(ns + "/bbx/plusY", params_.plusY_)) {
    ROS_WARN("No plusY value specified. Looking for %s",
             (ns + "/bbx/plusY").c_str());
    ret = false;
  }
  ...
  if (!ros::param::get(ns + "/bbx/softBounds", params_.softBounds_)) {
    ROS_WARN("Not specified whether scenario bounds are soft or hard. Looking "
  ...
  if (!ros::param::get(ns + "/system/bbx/x_offset",
                       params_.boundingBoxOffset_[0])) {
    ROS_WARN("No x offset value specified. Looking for %s. Default is 0.0m.",
             (ns + "/system/bbx/x_offset").c_str());
  }
  params_.boundingBoxOffset_[1] = 0.0;
  if (!ros::param::get(ns + "/system/bbx/y_offset",
                       params_.boundingBoxOffset_[1])) {
    ROS_WARN("No y offset value specified. Looking for %s. Default is 0.0m.",
             (ns + "/system/bbx/y_offset").c_str());
  }
  params_.boundingBoxOffset_[2] = 0.0;
  if (!ros::param::get(ns + "/system/bbx/z_offset",
                       params_.boundingBoxOffset_[2])) {
    ROS_WARN("No z offset value specified. Looking for %s. Default is 0.0m.",
             (ns + "/system/bbx/z_offset").c_str());
  }
  params_.cutoffIterations_ = 200;
  if (!ros::param::get(ns + "/vsep/tree/cutoff_iterations",
                       params_.cutoffIterations_)) {
    ROS_WARN(
        "No cutoff iterations value specified. Looking for %s. Default is 200.",
  ...
  if (!ros::param::get(ns + "/vsep/log/on", params_.log_)) {
    ROS_WARN("Logging is off by default. Turn on with %s: true",
             (ns + "/vsep/log/on").c_str());
  }
  params_.log_throttle_ = 0.5;
  if (!ros::param::get(ns + "/vsep/log/throttle", params_.log_throttle_)) {
    ROS_WARN("No throttle time for logging specified. Looking for %s. Default "
             "is 0.5s.",
  ...
  if (!ros::param::get(ns + "/pcl_throttle", params_.pcl_throttle_)) {
    ROS_WARN("No throttle time constant for the point cloud insertion "
             "specified. Looking for %s. Default is 0.333.",
             (ns + "/pcl_throttle").c_str());
  }
  params_.inspection_throttle_ = 0.25;
  if (!ros::param::get(ns + "/inspection_throttle",
                       params_.inspection_throttle_)) {
    ROS_WARN("No throttle time constant for the inspection view insertion "
             "specified. Looking for %s. Default is 0.1.",
             (ns + "/inspection_throttle").c_str());
  }
  params_.exact_root_ = true;
  if (!ros::param::get(ns + "/vsep/tree/exact_root", params_.exact_root_)) {
    ROS_WARN("No option for exact root selection specified. Looking for %s. "
             "Default is true.",
  ...
  if (!ros::param::get(ns + "/vsep/softStart", params_.softStart_)) {
    ROS_WARN("No option for soft starting (clearing a flight space of free "
             "cells until first successful iteration) specified. Looking for "
             "%s. Default is true.",
             (ns + "/vsep/softStart").c_str());
  }
  params_.softStartMinZ_ = 0.0;
  if (!ros::param::get(ns + "/vsep/softStartMinZ", params_.softStartMinZ_)) {
    ROS_WARN("No option for soft starting MinZ (clearing a flight space of "
             "free cells only over this MinZ value) specified. Looking for %s. "
             "Default is 0.0.",
  ...
  if (!ros::param::get(ns + "/vsep/curious_enable", params_.cuEnable_)) {
    ROS_WARN("Not specified whether activate 2nd layer or not. Looking for %s. "
             "Default is false",
             (ns + "vsep/curious_enable").c_str());
  }
  ...
  if (!ros::param::get(ns + "/vsep/curious_coefficient",
                       params_.curious_coefficient_)) {
    ROS_WARN("No gain range specified. Looking for %s. Default is 1.0m.",
             (ns + "/vsep/gain/range").c_str());
  }
  ...
  if (!ros::param::get(ns + "/vsep/extend_ratio_fixed",
                       params_.extend_ratio_fixed_)) {
    ROS_WARN("Not specified whether activate 2nd layer or not. Looking for %s. "
             "Default is false",
             (ns + "vsep/curious_enable").c_str());
  }
  ...
  if (!ros::param::get(ns + "/vsep/extend_ratio_max",
  ...
  if (!ros::param::get(ns + "/vsep/exp_lower_bound",
                       params_.exp_lower_bound_)) {
    ROS_WARN("Not specified whether activate 2nd layer or not. Looking for %s. "
             "Default is false",
             (ns + "vsep/curious_enable").c_str());
  }
  ...
  if (!ros::param::get(ns + "/vsep/exp_upper_bound",
                       params_.exp_upper_bound_)) {
    ROS_WARN("Not specified whether activate 2nd layer or not. Looking for %s. "
             "Default is false",
             (ns + "vsep/curious_enable").c_str());
  }
  ...
  if (!ros::param::get(ns + "/vsep/exp_filter_window",
                       params_.exp_filter_window_)) {
    ROS_WARN("Not specified whether activate 2nd layer or not. Looking for %s. "
             "Default is false",
             (ns + "vsep/curious_enable").c_str());
  }
  ...
  if (!ros::param::get(ns + "/vsep/sal_ground_remove_level",
                       params_.sal_ground_remove_level_)) {
    ROS_WARN("Not specified whether activate 2nd layer or not. Looking for %s. "
             "Default is false",
             (ns + "vsep/curious_enable").c_str());
  }
  ...
  if (!ros::param::get(ns + "/vsep/sal_wait_for_planner",
                       params_.sal_wait_for_planner_)) {
    ROS_WARN("Not specified whether activate 2nd layer or not. Looking for %s. "
             "Default is false",
             (ns + "vsep/curious_enable").c_str());
  ...
  if (!ros::param::get(ns + "/vsep/replanning_distance_min",
                       params_.vseReplanningDistanceMin_)) {
    ROS_WARN("No min distance threshold for Replanning step. Looking for %s. "
             "Default is 0.25.",
             (ns + "/vse/replanning_distance_min").c_str());
  }

  params_.vseReplanningExtensionRatio_ = 0.5;
  if (!ros::param::get(ns + "/vsep/replanning_extension_ratio",
                       params_.vseReplanningExtensionRatio_)) {
    ROS_WARN("No ratio for max extension range in Replanning step. Looking for "
             "%s. Default is 0.5.",
             (ns + "/vse/replanning_extension_ratio").c_str());
  }

  params_.planMarker_lifetime_ = 15.0;
  if (!ros::param::get(ns + "/markers/planMarker_lifetime",
                       params_.planMarker_lifetime_)) {
    ROS_WARN("No lifetime for planning markers specified. Looking for %s. "
             "Default is 15.0s.",
             (ns + "/markers/planMarker_lifetime").c_str());
  }
  params_.bestPlanMarker_lifetime_ = 15.0;
  if (!ros::param::get(ns + "/markers/bestPlanMarker_lifetime",
                       params_.bestPlanMarker_lifetime_)) {
    ROS_WARN("No lifetime for Best planning markers specified. Looking for %s. "
             "Default is 15.0s.",
             (ns + "/markers/bestPlanMarker_lifetime").c_str());
  }
  params_.replanMarker_lifetime_ = 15.0;
  if (!ros::param::get(ns + "/markers/replanMarker_lifetime",
                       params_.replanMarker_lifetime_)) {
    ROS_WARN("No lifetime for replanning markers specified. Looking for %s. "
             "Default is 15.0s.",
             (ns + "/markers/replanMarker_lifetime").c_str());
  }
  params_.bestReplanMarker_lifetime_ = 15.0;
  if (!ros::param::get(ns + "/markers/bestReplanMarker_lifetime",
                       params_.bestReplanMarker_lifetime_)) {
    ROS_WARN("No lifetime for Best replanning markers specified. Looking for "
             "%s. Default is 15.0s.",
             (ns + "/markers/bestReplanMarker_lifetime").c_str());
  }

  if (!ros::param::get(ns + "/bbx/explorationExtensionX",
                       params_.explorationExtensionX_)) {
    ROS_WARN("No BBx x-extension value for IG collection (exploration space) "
             "specified. Looking for %s",
             (ns + "/bbx/explorationExtensionX").c_str());
    ret = false;
  }
  if (!ros::param::get(ns + "/bbx/explorationExtensionY",
                       params_.explorationExtensionY_)) {
    ROS_WARN("No BBx y-extension value for IG collection (exploration space) "
             "specified. Looking for %s",
             (ns + "/bbx/explorationExtensionY").c_str());
    ret = false;
  }
  if (!ros::param::get(ns + "/bbx/explorationMinZ", params_.explorationMinZ_)) {
    ROS_WARN("No BBx z-min value for IG collection (exploration space) "
             "specified. Looking for %s",
             (ns + "/bbx/explorationMinZ").c_str());
    ret = false;
  }
  if (!ros::param::get(ns + "/bbx/explorationMaxZ", params_.explorationMaxZ_)) {
    ROS_WARN("No BBx z-max value for IG collection (exploration space) "
             "specified. Looking for %s",
             (ns + "/bbx/explorationMaxZ").c_str());
    ret = false;
  }
  ...
  if (last + params_.pcl_throttle_ < ros::Time::now().toSec()) {
    ...
    if (params_.softStart_ && params_.planner_state_ <= PSTATE_BBXCLEARANCE) {
      Eigen::Vector3d boundingBoxSize(
          params_.boundingBox_[0] + 5.0 * params_.dOvershoot_,
          params_.boundingBox_[1] + 5.0 * params_.dOvershoot_,
          params_.boundingBox_[2] + 5.0 * params_.dOvershoot_);
      // const double minZ = 2.0*manager_->getResolution();
      tree_->clearRootStateBBX(boundingBoxSize,
                               params_.softStartMinZ_); // minZ);
    }
  }
}
